...
bool ASC_Output_WriteUniverseSnapshotMirror(const ASC_RuntimeConfig &config,const ASC_SymbolRecord &records[],const int count)
  {
   ...
   if(handle == INVALID_HANDLE)
     {
      ASC_Logger_Log("ERROR","OUTPUT","ASC_Output_WriteUniverseSnapshotMirror","failed opening mirror file");
      return(false);
     }
   ...
   if(!ASC_Output_WriteSymbolSurfaces(config,records,count))
     {
      ASC_Logger_Log("ERROR","OUTPUT","ASC_Output_WriteUniverseSnapshotMirror","failed writing symbol dossiers");
      return(false);
     }
   ...
   if(!ASC_Output_WriteSummarySurface(config,records,count))
     {
      ASC_Logger_Log("ERROR","OUTPUT","ASC_Output_WriteUniverseSnapshotMirror","failed writing summary surface");
      return(false);
     }

   ASC_Logger_Log("INFO","OUTPUT","ASC_Output_WriteUniverseSnapshotMirror",
                  "mirror+dossiers+summary written count=" + IntegerToString(count));
