# Aurora Office Task Board

## Current posture

- Aurora now has enough doctrine, bridge, ledger, and wrapper surfaces that compact office control is required.
...
|---|---|---|---|
| Office control | ACTIVE | keep Aurora control compact, canonical, and low-drift | maintain office files and checkpoint only meaningful changes |
| Control-file hardening | ACTIVE | keep control index, recovery order, and tracker aligned to current repo truth | retire stale references and classify active vs historical surfaces explicitly |
...
| Risky merge cleanup | BLOCKED | merge overlapping historical files only when clearly lossless | prefer cross-linking over destructive cleanup |
...
Do not:
- delete lineage-rich files for neatness
- treat older generations as active when newer canonical generations exist
...
Update this board instead of spawning new control notes for each small Aurora step.
