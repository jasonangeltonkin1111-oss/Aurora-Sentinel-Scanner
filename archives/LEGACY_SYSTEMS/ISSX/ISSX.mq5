...
bool ISSX_WriteEA1OperatorStatus(const string lifecycle_state,const string readiness_reason)
  {
   if(ISSX_Util::IsEmpty(g_market_json_relative_path))
     {
   ...
   ISSX_DataHandler::ForensicState fs_market;
   fs_market.Reset();
   const bool ok=ISSX_DataHandler::WritePayloadAtomic(g_market_json_relative_path,json,fs_market,true);
   ...
   g_debug.Write((ok?"INFO":"ERROR"),
                 ...
                 " final_path="+g_market_json_relative_path);
   return ok;
  }
...
if(!ISSX_ValidateStageRegistry(stage_registry_reason))
  {
   ...
   ISSX_WriteEA1OperatorStatus("attached","oninit_ready");
   ISSX_RenderHudCycle();
